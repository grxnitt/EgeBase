...
export function generateStaticParams() {
  return getPublishedArticles("sociology").map((article) => ({ slug: article.meta.slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const article = getArticleBySlug(slug, "sociology");
  if (!article) {
    return {};
  }

  const canonical = `/theory/sociology/${article.meta.slug}`;
...
export default async function ArticlePage({ params }: PageProps) {
  ...
  const allArticles = getPublishedArticles("sociology");
  const adjacent = getAdjacentArticles(article);
  const canonical = `${siteConfig.url}/theory/sociology/${article.meta.slug}`;
  const articleJsonLd = {
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Теория", item: `${siteConfig.url}/theory` },
      {
        ...
        position: 2,
        name: "Социология",
        item: `${siteConfig.url}/theory/sociology`
      },
    ...
    <div className="container-shell py-12">
      ...
      <Breadcrumbs
        items={[
          ...
          { label: "Социология", href: "/theory/sociology" },
      ...
      <article className="grid grid-cols-[minmax(0,1fr)_240px] gap-12 py-14">
        <div>
          ...
          <ArticleUserActions
            ...
            returnTo={`/theory/sociology/${article.meta.slug}`}
          />
          ...
          >
            <div className="article-adjacent-item group border-t border-transparent pt-4">
              ...
              {adjacent.previous ? (
                <TextLink
                  ...
                  href={`/theory/sociology/${adjacent.previous.meta.slug}`}
                >
            ...
            <div className="article-adjacent-item group border-t border-transparent pt-4 text-right">
              ...
              {adjacent.next ? (
                <TextLink
                  className="mt-2 inline-flex items-center gap-2 decoration-transparent"
                  href={`/theory/sociology/${adjacent.next.meta.slug}`}
                >
                  <span>{adjacent.next.meta.title}</span>
                  <span aria-hidden="true" className="motion-arrow">→</span>
                </TextLink>
              ) : (
                <span className="mt-2 block text-sm text-muted">Это последняя опубликованная тема.</span>
              )}
            </div>
          </nav>
        </div>
